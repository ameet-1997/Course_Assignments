...
xgb1.fit(pca.transform(train_feat), train_labels, verbose=True)

# print("Total time: "+str(time.time()-start_time))
predicted_labels = xgb1.predict(pca.transform(validation_feat))
...
np.savetxt('xgboost_validation.csv',to_write1,delimiter=',',header='id,label',fmt='%d',comments='')

# Writing Test Output
pred_test = xgb1.predict(pca.transform(test_data_org))
...
np.set_printoptions(suppress=True)
np.savetxt('xgboost_test.csv',to_write,delimiter=',',header='id,label',fmt='%d',comments='')
...
bag = BaggingClassifier(base_estimator=mlp, n_estimators=200, max_samples=1.0, max_features=1.0, bootstrap=True, bootstrap_features=True, oob_score=False, warm_start=False, n_jobs=-1, random_state=None, verbose=True)
